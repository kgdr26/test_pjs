...
class InboundController extends Controller
{
    ...
    function inbound(): object {
        ...
        return view('Inbound.list')->with($data);
    }

    function listdata(Request $request): object{
        $query = DB::table('trx_inbound as ti')
            ->select(
                'ti.id_transaction',
                'ti.date',
                'ti.no_pr',
    ...
    function inbound_data_tools(Request $request): object{
        $id = $request->id_tools;
    ...
    function generate(Request $request): object{
        ...
        $query['tools'] = DB::table('mst_tools as mt')
            ...
            $jml = DB::table('trx_inbound')->count();
            $query['id_transaction'] = 'TI-BBN.'.str_pad(($jml+1), 3, '0', STR_PAD_LEFT) . "\n";
